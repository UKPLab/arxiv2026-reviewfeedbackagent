...
template_csv = "full-{}-advanced_features.tsv"
for llm in llms:
    ...
    full.to_csv(csv_file, sep="\t")


def main():
    """
    Main function to generate feature vectors from a dataset using specified LLM.
    
    Accepts command-line arguments:
        --data_file: Path to the data file (TSV format)
        --feature_questions_file: Path to the feature questions pickle file
        --llm_name: Name or path of the LLM model to use
    """
    parser = argparse.ArgumentParser(
        description="Generate feature vectors from review segments using an LLM"
    )
    parser.add_argument(
        "--data_file",
        type=str,
    ...
    parser.add_argument(
        "--feature_questions_file",
        type=str,
        required=True,
        help="Path to the feature questions pickle file"
    )
    parser.add_argument(
        "--llm_name",
        type=str,
    ...
    # Load feature questions from pickle file
    print(f"Loading feature questions from: {args.feature_questions_file}")
    feature_questions = pd.read_pickle(args.feature_questions_file)
    
    # Generate feature vectors
    feature_vectors = []
    print("Generating feature vectors...")
    for index, row in tqdm(data.iterrows(), desc="Generating feature vectors", total=len(data)):
        review_segment = row["Review Segment"]
        feature_vectors.append(generate_feature_vectors(review_segment, feature_questions, llm=args.llm_name))
    
    # Add vectors to dataframe and save
    data['feature_vectors'] = feature_vectors
    output_file = f"full-{safe_str(args.llm_name)}-advanced_features.tsv"
